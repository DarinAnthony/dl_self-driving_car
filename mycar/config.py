...
PWM_STEERING_THROTTLE = {
    ...
    "PWM_STEERING_INVERTED": True,         # True if hardware requires an inverted PWM pulse
    "PWM_THROTTLE_PIN": "PIGPIO.BCM.13",   # PWM output pin for ESC
    "PWM_THROTTLE_SCALE": 1.0,              # used to compensate for PWM frequence differences from 60hz; NOT for increasing/limiting speed
    "PWM_THROTTLE_INVERTED": False,         # True if hardware requires an inverted PWM pulse
    "STEERING_LEFT_PWM": 200,             # 1ms pulse width for full left
    "STEERING_RIGHT_PWM": 400,            # 2ms pulse width for full right
    "THROTTLE_FORWARD_PWM": 400,          # 2ms pulse width for full forward
    "THROTTLE_STOPPED_PWM": 300,          # 1.5ms pulse width for neutral
    "THROTTLE_REVERSE_PWM": 200           # 1ms pulse width for full reverse
}
